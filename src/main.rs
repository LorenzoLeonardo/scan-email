...
fn main() {
    ...
    let build_ver_minor = 1;
    let build_revision = 1;
    let version_code: i32 = build_ver_major << 24 | build_ver_minor << 16 | build_revision & 0xffff;
    let sane = Sane::init(version_code).expect("Failed to initialize SANE");
    let mut devices = sane
        .get_devices(false)
